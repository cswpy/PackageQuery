...
double DynamicLowVariance::kGroupRatio = 0.1;
...
DynamicLowVariance::DynamicLowVariance(string dbname): dbname(dbname){
  vector<string> names = {"Init", "FullPartition", "PartialPartition", "3", "4", "5", "6"};
  pro = Profiler(names);
  init();
}
...
void DynamicLowVariance::doPartition(string table_name, string suffix, const vector<string> &cols){
  ...
  #pragma omp parallel num_threads(kPCore)
  {
    ...
    #pragma omp master
    {
      ...
      atts_names += "max_total_var DOUBLE PRECISION, max_size BIGINT, status INTEGER";
    ...
    #pragma omp master
    {
      ...
      sql = fmt::format("CREATE INDEX \"{}_id_index\" ON \"{}\"(id)", g_name, g_name);
      res = PQexec(conn, sql.c_str());
      PQclear(res);
      ...
      sql = fmt::format("CREATE INDEX \"{}_gid_index\" ON \"{}\"(gid)", p_name, p_name);
      res = PQexec(conn, sql.c_str());
      PQclear(res);
