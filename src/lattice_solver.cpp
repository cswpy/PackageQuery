...
VectorXd LatticeSolver::uncrushX(VectorXd pre_x){
...
void LatticeSolver::initialize(MatrixXd A, VectorXd b, VectorXd c, VectorXd lb, VectorXd ub){
  int status = 0;
...
void LatticeSolver::extractModel(CPXCLPptr model){
...
void LatticeSolver::solveRelaxed(){
  exe_tableau = 0;
  exe_presolve = 0;
  exe_relaxed = 0;
  // Debug
...
LatticeSolver::LatticeSolver(MatrixXd A, VectorXd b, VectorXd c, VectorXd lb, VectorXd ub){
  exe_init = exeTime([](LatticeSolver* ls, MatrixXd& A, VectorXd& b, VectorXd& c, VectorXd& lb, VectorXd& ub){
    ls->initialize(A, b, c, lb, ub);
  }, this, A, b, c, lb, ub);
}

LatticeSolver::LatticeSolver(MatrixXd A, VectorXd b, VectorXd c, double lb_value, double ub_value){
...
LatticeSolver::LatticeSolver(MatrixXd A, VectorXd b, VectorXd c): LatticeSolver(A, b, c, 0, DBL_MAX){
}

LatticeSolver::LatticeSolver(string problem){
  int status = 0;
  env = CPXXopenCPLEX(&status); assert(!status);
  model = CPXXcreateprob(env, &status, problem.c_str()); assert(!status);
...
bool LatticeSolver::checkFeasibilityStep(const VectorXd& current_feas_scores, const unordered_map<int,int>& bound_map){
...
bool LatticeSolver::latticeStep(int d, VectorXd& x, VectorXd& current_feas_scores, double& current_c_score, unordered_map<int,int>& bound_map){
  int i = abs(d) - 1;
  int s = sign(d);
  x(i) += s;
